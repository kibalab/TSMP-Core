...
通常の Unity と VRChat のどちらでも、次のプレハブをシーンにドラッグします。

```text
Packages/com.kibalab.tsmp.core/Samples/TSMPController.prefab
```

Setup がエンコーダー、デコーダーとインストール済みコーデックを自動で準備します。変換メニューや実行環境の選択は不要です。要件と Player 設定は [インストール](installation.md) を参照してください。
...
`TSMPSetup` でコーデックと入出力の参照を確認します。コンポーネント、コーデック、バインディングの変更は自動反映され、Play モードやビルドの前にも確認されます。既存の `Apply Setup` ボタンは任意の手動更新であり、必須の手順ではありません。
...
Interact テストは VRChat 用です。通常の Unity では、まず `TSMPNetworkTransformSync` の対象をアニメーションやスクリプトで動かして確認します。RPC は UI ボタンやスクリプトから `SendTransRPC` を呼び出してテストできます。Unity 自体は VRChat の Interact イベントを発行しません。
